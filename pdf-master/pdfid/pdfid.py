...
class cEntropy:
    ...
    def calc(self):
        ...
        if streamCount == 0:
            return (allCount, sum([fEntropy(x, allCount) for x in self.allBucket]), streamCount, None,
                    nonStreamCount, sum([fEntropy(x, nonStreamCount) for x in self.nonStreamBucket]))
        else:
            return (allCount, sum([fEntropy(x, allCount) for x in self.allBucket]), streamCount,
                    sum([fEntropy(x, streamCount) for x in self.streamBucket]), nonStreamCount,
                    sum([fEntropy(x, nonStreamCount) for x in self.nonStreamBucket]))
